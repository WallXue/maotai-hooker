...
public class JingDongHooker implements IXposedHookLoadPackage {


    @Override
    public void handleLoadPackage(XC_LoadPackage.LoadPackageParam lpparam) throws Throwable {
        XposedBridge.log("加载APP: " + lpparam.packageName);
        if (!JDConstants.JD_PKG_NAME.equals(lpparam.packageName)) {
            return;
        }
        final ClassLoader appClassLoader = lpparam.classLoader;
        HookHelper.hiddenXposedBridge();
        HookHelper.hiddenJDXposedTip(appClassLoader);
//        HookHelper.showActivityNameHelper(appClassLoader);
        ...
        XposedHelpers.findAndHookMethod(Activity.class.getName(), appClassLoader,
                "onPostResume", new XC_MethodHook() {
                    @Override
        ...
        XposedHelpers.findAndHookMethod("com.jingdong.app.mall.JDApp", appClassLoader,
